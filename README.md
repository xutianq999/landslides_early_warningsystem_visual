...
**在线监护**用这个入口:读 `config.json` 的设备表 + 工作台导出的 `profiles/*.json`,
把两套通道跑起来,盯住变化率与报警等级。
...
| 界面区域 | 说明 |
|---|---|
| 运行配置 | 算法配置文件(下拉 `profiles/*.json`)、分析间隔/降雨加密间隔/降雨触发、分割尺寸、是否剔除动态物体、推理设备(mps/cpu)、**临时视频源** |
| 设备表 | 每行一个点位:「启动」勾选框 + 「实时」「分析」两个通道开关(运行中可直接勾掉,立即生效,不用重启) |
| 实时列 | 状态(预热% / 正常 / 各色报警 / 相机异常)、变化率、速率、加速度、有效像素、基线进度、帧源(连接/帧数/错误数)、最近分析 |
| 报警横幅 | 取所有点位的最高等级,红/橙/黄/绿 + 相机异常单独提示 |
| 运行日志 | 帧源重连、报警存图、分析完成、异常堆栈都转发到这里 |

- **与命令行同一套代码**:界面调用的就是 `monitor.py` 的 `make_runtime`/`DeviceRuntime`,
  所以 GUI 与 `monitor.py` 的参数口径、判定结果完全一致;区别只是 GUI 里能随时开关通道。
- **无相机也能试跑**:「临时视频源」填一段本地视频(或图片)路径,把所有点位的取流都换成它。
  注意实时通道开始判定前要攒够基线历史(默认 10 分钟),试跑时把配置文件里的
  `realtime.baseline_min` 改成 `0.1` 就能几十秒内看到变化率与报警。
- 只有"分析"通道落库、只有报警存图,和命令行完全一样;报警证据在「打开报警证据目录」里。

### 各入口的职责(别再混着用)

| 入口 | 定位 |
|---|---|
| `studio.py` | **离线调参**:对一张图试分割/深度/参数,导出 `profiles/*.json` |
| `runtime.py` | **在线监护(GUI)**:启停两套通道、看实时指标与报警(读同一份配置) |
| `monitor.py` | 在线监护(命令行):无人值守/launchd 常驻用这个 |
| `app.py` | 浏览器版(可远程访问),功能子集 |
| `features.py` / `alarm.py` | 命令行/批量处理,可脚本化 |
| `capture.py` | 只跑定时抓图这一个通道 |
| `api.py` | 对外数据接口,供平台拉取 |
...
- **同一帧只判一次**:判定频率可能高于抽帧频率(监视台 2 Hz 轮询、抽帧 1 fps),
  同一帧重复判定会让 dt 退化成 1 ms,速率被算成天文数字 → 假红警;按帧时间戳去重。
- **只报上升**:速率突增或加速为正才报警。用绝对值会把"事件后的回落"也判成红警;
  二阶差分的正尖峰在事件**回落**时也会出现(中值滤波滚出事件),所以加速度规则要求
  速率同时为正才算数。
- **阈值比较用平滑后的值**:中值滤波滞后于瞬时值,判定用平滑值、日志也报平滑值,
  否则会出现"变化率 0.0% 连续 2 次 ≥ 12.0%"这种自相矛盾的告警理由。
- **相机被碰动**表现为全图都在变 → 配准位移超阈时标记为"相机异常",不计入滑坡判定。
- **清晰度门控在全分辨率上算**:拉普拉斯方差随尺度变化很大,在下采样图上算会把正常帧误判成失焦。
  门控把帧判成"失焦"时实时通道会一条判定都不做(监视台的状态列会一直停在"预热"),这是
  设计行为;若现场子码流就是低纹理(水泥面/雾天),把 `gate.blur_min` 调低再标定。
...
studio.py              桌面工作台入口(PySide6):分割/深度/特征/抓图/数据(离线调参)
runtime.py             运行时监视台入口(PySide6):启停两套通道、看实时指标与报警(在线监护)
gui/                   工作台界面:主窗口、后台任务、图像查看器、各标签页
gui/runtime_window.py  监视台界面:设备表 / 通道开关 / 报警横幅 / 日志
